...
def test_get_user_by_username_db(mock_db_session):
    ...
    result = get_user_by_username_db(mock_username, mock_db_session.return_value)
    assert result[0].username == "test_user10"


def test_create_new_user_db(mock_db_session):
    ...
    result = create_new_user_db(mock_user, mock_db_session)
